...
In fact, key rotation is already enabled! [`securekey`](https://elements.heroku.com/addons/securekey) only has one plan, _Fortnightly_, so all we have to do is wait 2 weeks, and the key will be rotated for both services!
...
## _(Bonus) Graceful key rotation_
